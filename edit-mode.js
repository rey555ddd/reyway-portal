...
async function loadOverrides() {
  try {
    ...
    for (const [key, text] of Object.entries(items)) {
      if (key.endsWith('--fs')) {
        // 字體大小覆寫
        const baseKey = key.slice(0, -4); // 移除 --fs
        const el = document.querySelector(`[data-edit-id="${baseKey}"]`);
        if (el && text) el.style.fontSize = text;
      } else {
        // 原有文字覆寫
...
// ── 字體大小工具列 ──
let currentFsEl = null; // 目前選中的元素

function getFsToolbarHTML() {
  return `<div id="fs-toolbar">
    <span class="fs-label" id="fs-el-label">—</span>
    <button class="fs-btn" id="fs-btn-minus">A-</button>
    <span class="fs-value" id="fs-current-val">—</span>
    <button class="fs-btn" id="fs-btn-plus">A+</button>
    <span class="fs-reset" id="fs-reset">重設</span>
    <span class="fs-close" id="fs-close">×</span>
  </div>`;
}

function showFsToolbar(el) {
  const toolbar = document.getElementById('fs-toolbar');
  if (!toolbar) return;
  currentFsEl = el;
  const key = el.getAttribute('data-edit-id');
  document.getElementById('fs-el-label').textContent = key;
  const currentFs = parseFloat(window.getComputedStyle(el).fontSize);
  document.getElementById('fs-current-val').textContent = Math.round(currentFs) + 'px';
  toolbar.classList.add('visible');
}

function hideFsToolbar() {
  const toolbar = document.getElementById('fs-toolbar');
...
function adjustFontSize(delta) {
  if (!currentFsEl) return;
  const current = parseFloat(window.getComputedStyle(currentFsEl).fontSize);
  const newSize = Math.max(8, Math.round(current + delta));
  currentFsEl.style.fontSize = newSize + 'px';
  document.getElementById('fs-current-val').textContent = newSize + 'px';
  saveFontSize(currentFsEl, newSize);
}

const fsTimers = new Map();
function saveFontSize(el, size) {
  const key = el.getAttribute('data-edit-id') + '--fs';
  clearTimeout(fsTimers.get(key));
  fsTimers.set(key, setTimeout(async () => {
    try {
      const r = await fetch('/api/text', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'X-Owner-Token': OWNER_TOKEN },
        body: JSON.stringify({ key, text: size + 'px' }),
      });
      if (!r.ok) throw new Error('HTTP ' + r.status);
      showToast('字體大小已儲存 ✓');
    } catch (e) {
      console.error('[edit-mode] saveFontSize failed:', e);
      showToast('字體大小儲存失敗 · ' + e.message, true);
    }
  }, 600));
}

async function resetFontSize(el) {
  if (!el) return;
  const key = el.getAttribute('data-edit-id') + '--fs';
  el.style.fontSize = '';
  document.getElementById('fs-current-val').textContent = Math.round(parseFloat(window.getComputedStyle(el).fontSize)) + 'px';
  try {
    const r = await fetch('/api/text', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'X-Owner-Token': OWNER_TOKEN },
      body: JSON.stringify({ key, text: '' }), // 空字串 = 清除
    });
    if (!r.ok) throw new Error('HTTP ' + r.status);
    showToast('已重設字體大小');
  } catch (e) {
    console.error('[edit-mode] resetFontSize failed:', e);
    showToast('重設失敗 · ' + e.message, true);
  }
}
...
async function init() {
  ...
  // 注入字體大小工具列 HTML（CSS 在 index.html 的 <style> 中）
  document.body.insertAdjacentHTML('beforeend', getFsToolbarHTML());

  // 掛工具列按鈕事件
  document.getElementById('fs-btn-minus').onclick = () => adjustFontSize(-1);
  document.getElementById('fs-btn-plus').onclick  = () => adjustFontSize(+1);
  document.getElementById('fs-reset').onclick     = () => resetFontSize(currentFsEl);
  document.getElementById('fs-close').onclick     = hideFsToolbar;
  ...
  // 為每個可編元素掛失焦 + 鍵盤快捷（含 IME composition 防護）+ 字體工具列 click
  document.querySelectorAll('[data-edit-id]').forEach(el => {
    ...
    // 點選元素時，若編輯模式 ON，顯示字體大小工具列
    el.addEventListener('click', () => {
      if (document.body.classList.contains('edit-on')) showFsToolbar(el);
    });
  });
